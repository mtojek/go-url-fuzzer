...
var (
	headers            = kingpin.Flag("header", "Header=value, custom HTTP header added to every fuzz request").Short('h').StringMap()
	methods            = kingpin.Flag("method", "HTTP method used in tests (GET, POST, PUT, DELETE, HEAD, OPTIONS)").Short('m').Default("GET").Strings()
	outputFile         = kingpin.Flag("output", "Output text file with found urls and statuses").Short('o').String()
	reportDirectory    = kingpin.Flag("report", "Target output directory of an url fuzzing HTML report").Short('r').File()
